...
- **`projects.default_scheduler_owner` column** lets each project pick
  its preferred scheduler. New schedules created with `scheduler=None`
  (the default for the declarative reconcilers) fall back to this
  value at row-create time. Default `"z4j-scheduler"`; celery-beat-
  first shops can flip via `PATCH /api/v1/projects/{slug}`. Migration
  `2026_04_30_0014_proj_sched_owner`.

  **Important semantic** (round-9 root-cause clarification):
  flipping `default_scheduler_owner` only affects schedules created
  AFTER the change. Existing rows keep the value they were created
  with. The next reconciler `:import` for those rows resolves to the
  NEW default and treats the OLD-default rows as absent — under
  `replace_for_source` mode that means they get DELETED. Operators
  who want to retroactively migrate existing rows use the new
  `z4j-brain projects rewrite-scheduler --slug X --from A --to B`
  CLI command (audit-logged, scoped to declarative-source rows by
  default; pass `--all-sources` to widen). Earlier 1.2.2 builds
  tried to auto-migrate at PATCH time and the resulting cascade of
  concurrency / lock / staleness bugs across rounds 3-8 of the audit
  was the reason for the round-9 root-cause revert.

- **`z4j-brain projects rewrite-scheduler` CLI** for the explicit
  retroactive migration described above. Supports `--dry-run` to
  preview the row count, `--all-sources` to widen beyond the
  declarative/imported source-label scope. Audit-logged with
  `action="project.rewrite_scheduler"` + scope + count metadata.

- **Audit-log retention sweeper** prunes `audit_log` rows older than
  `audit_retention_days` (default 90). Runs every
  `audit_retention_sweep_interval_seconds` (default 3600) on a
  background task, deleting in batches. Postgres uses a session-scoped
  GUC opt-in (`SET LOCAL z4j.audit_sweep = 'on'`) so the append-only
  trigger still blocks every other code path. Migration
  `2026_05_01_0015_audit_sweep`.

- **SQLite WAL checkpoint task** runs `PRAGMA wal_checkpoint(TRUNCATE)`
  every 5 minutes by default. Keeps the `*.db-wal` sidecar bounded on
  busy homelab boxes. No-op on Postgres.

- **Brain self-watch Prometheus metrics**: `/metrics` now exposes
  `z4j_audit_retention_pruned_total`, `z4j_audit_retention_last_run_timestamp`,
  `z4j_wal_checkpoint_pages_last`, `z4j_wal_checkpoint_last_run_timestamp`,
  and `z4j_background_task_errors_total{task=...}`.

- **`z4j upgrade` CLI**: lists installed z4j-* packages and the latest
  version on PyPI; `--apply` runs `pip install -U z4j`; `--json` for
  machine-readable output.

- **Dashboard: Scheduler column** added to the schedules table.

### Changed

- Schedule creation now treats `scheduler=None` as "use the project's
  `default_scheduler_owner`". Explicit `scheduler=...` still works.

### Robustness (1.2.2 deep audit fixes)

After the initial implementation we ran a 5-agent deep audit
(security, concurrency, migrations, API/IDOR, performance) and
fixed every CRITICAL + HIGH + MEDIUM finding before the final
build. Highlights:

- **`audit_log.api_key_id` attribution** (HIGH-11). New nullable
  column + migration `0017`. HMAC canonical bumped to **v4** so
  swapping `api_key_id` without re-signing breaks `verify_row()`;
  fallback chain v4 → v3 → v2 → v1 preserves pre-1.2.2 row
  verification. Schedule import handler now threads
  `request.state.api_key.id` into `AuditService.record()`.
- **`projects.allowed_schedulers` allow-list** (MED-13). New
  optional JSON column + migration `0018`. `NULL` = unrestricted
  (default); array enforces membership across schedule
  create + per-row import. Default scheduler is implicitly
  always allowed.
- **`ix_audit_log_occurred_at` index** (CRIT-3) + migration
  `0016`. Retention sweeper's range scan is now index-backed.
- **Audit-log retention sweeper** rewritten for per-batch
  transactions (CRIT-4), Postgres advisory lock so multi-worker
  deployments don't fight (MED-15), `audit_retention_sweep_max_per_pass`
  cap (MED-18), CancelledError re-raise (CRIT-2).
- **WAL checkpoint** defaults to PASSIVE (MED-14); waits before
  the inaugural pass (MED) so a 2GB WAL on first boot doesn't
  stall startup.
- **`/metrics` self-watch** uses Gauges (not synthetic-delta
  Counters) so multi-replica Prometheus scrapes are idempotent
  and tests don't carry baseline state across `create_app()`
  calls (CRIT-5).
- **FastAPI lifespan** wraps the sync-httpx reconciler call in
  `asyncio.to_thread` so a slow brain doesn't block the event
  loop during boot (CRIT-6).
- **Migration `0014`** self-heals partial-fail state (backfills
  NULLs, tightens nullable on Postgres) (CRIT-7) and guards
  SQLite < 3.35 with a clean error.
- **Migration `0015`** revokes EXECUTE on the trigger function
  from PUBLIC (MED-17). New regression test guards the SQL
  contract against future drift.
- **Schedule import** caps `body.schedules` at 2000 rows
  (HIGH-8) to bound the `replace_for_source` blast radius.
- **Reconciler hardening** (HIGH-9, HIGH-10): project_slug is
  validated against the brain's exact regex + URL-quoted in the
  path; response bodies in error logs are redacted of
  Bearer/secret/token strings; +1 transport-level retry on
  transient connection errors.
- **`z4j upgrade` CLI** has a walltime budget cap and aggregates
  errors across all packages (HIGH-12).
