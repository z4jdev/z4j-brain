...
def _run_serve(args: argparse.Namespace) -> int:
    ...
    if os.environ.get("Z4J_AUTO_MIGRATE", "true").lower() != "false":
        try:
            ...
            # ``z4j-brain migrate sync --allow-future-schema``.
            print(  # noqa: T201
                f"z4j-brain: DB is at a NEWER alembic head "
                f"({exc.db_head!r}) than this code knows about. "
                "Continuing boot; this code will serve the subset "
                "of features it understands. Run `z4j-brain "
...
def _run_migrate_sync(
    ...
    *,
    ...
    if db_head is None:
        ...
        print(  # noqa: T201
            f"z4j-brain migrate sync: DB head is known; running "
            f"`alembic upgrade head` (target={code_head!r}).",
        )
        alembic_main(
            argv=["-c", str(config_path), "upgrade", "head"],
            prog="z4j-brain migrate sync",
        )
        return 0

    # DB has a future revision. Refuse without explicit consent.
    print(  # noqa: T201
        f"z4j-brain migrate sync: DB is at FUTURE revision {db_head!r} "
    ...
    if not (allow_future and confirm_destructive):
        print(  # noqa: T201
            "z4j-brain migrate sync: refusing to proceed. To stamp "
            "the DB back to this code's head AND drop the tables "
            "the newer code added, re-run with both:\n"
            "    --allow-future-schema\n"
            "    --i-know-this-can-corrupt-data\n"
            "Recommended alternative: install z4j-brain at the "
    ...
    print(  # noqa: T201
        f"z4j-brain migrate sync: STAMPING DB back to {code_head!r} "
    ...
    alembic_main(
        ...
        prog="z4j-brain migrate sync (stamp)",
    )
    ...
    if db_url:
        ...
        try:
            ...
            engine.dispose()
    print(  # noqa: T201
        "z4j-brain migrate sync: done. Restart the brain.",
    )
    return 0
